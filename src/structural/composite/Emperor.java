...
public class Emperor extends Title {
    public Emperor(String name, String responsibility) {
        super(name, responsibility);
    }

    @Override
    public void addReport(Title title) {  // 可以在abstract的接口上细化类型
        if (title instanceof Department) {
    ...
    public Iterator<Title> iterator() {
        return new TitleLevelorderIterator(this);
    }
